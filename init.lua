...
function M.project_findFiles2(projectDef)
  ...
  for i,f in ipairs(projectDef.exclude) do
    if string.len(egrep) > 1 then
      egrep = egrep..'|'..f
    else
      egrep = f
    end
  ...
  if string.len(egrep) < 2 then
    egrep="~~~~" -- dummy string
  end

  for i,f in ipairs(projectDef.types) do
    if string.len(findTypes) > 1 then
      findTypes = findTypes..' -o -name "*.'..f..'" '
    else
      findTypes = '-name "*.'..f..'" '
    end
  end
  ...
  local cmd = ""
  if string.len(findTypes) > 1 then
    cmd = '\\( '..findTypes..' \\) | egrep -iv "('..egrep..')"'
  else
    cmd = ' | egrep -v "('..egrep..')"'
  end

  for i,f in ipairs(projectDef.folders) do
    print('find '..f..' -type f '..cmd)
    local p = io.popen('find '..f..' -type f '..cmd)
